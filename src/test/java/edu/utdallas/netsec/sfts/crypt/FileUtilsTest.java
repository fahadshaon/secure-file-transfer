...
public class FileUtilsTest {
    ...
    public void testIsFileInDirectory() throws Exception {
        ...
        Assert.assertTrue(FileUtils.isFileInDirectory("data/test", "config"));
        Assert.assertTrue(FileUtils.isFileInDirectory("data/test", "/./config"));

        Assert.assertFalse(FileUtils.isFileInDirectory("data/test", "/../../config"));
        Assert.assertFalse(FileUtils.isFileInDirectory("data/test", "/../config"));
    }

    @Test
    public void testGetNonExistingFilename() throws Exception {

        Assert.assertEquals(FileUtils.getNonExistingFilename("data/test"), "data/test.1");
        Assert.assertEquals(FileUtils.getNonExistingFilename("data/test/config"), "data/test/config.1");
        Assert.assertEquals(FileUtils.getNonExistingFilename("data/test/test.txt"), "data/test/test.txt");
    }
}

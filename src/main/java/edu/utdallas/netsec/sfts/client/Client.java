...
import edu.utdallas.netsec.sfts.crypt.*;
...
public class Client {
    ...
    public void run() throws IOException, ClassNotFoundException {
        ...
        if (dClientResponse.respCode == 0 && MessageDigest.isEqual(dClientResponse.nonce, deptRequest.nonce)) {
            ...
            if (!FileUtils.isFileInDirectory(basePath, this.fileName)) {
                throw new RuntimeException("Seems like provided filename is tricky. Exiting for safety");
            }

            String fileName = FileUtils.getNonExistingFilename(basePath + "/" + this.fileName);
